"""gen_reaction_1d.py — Conditional generation from a trained 1D Reaction-Diffusion
Factor-DiT checkpoint.

Mirrors `1d_burgers/generate/gen_burgers_1d.py`, with the only difference that
the AdaLN conditioning is 2D ``(log_nu, log_rho)`` instead of scalar ``log_nu``.

For each seed in --seeds, iterate the test dataset (500 samples) with batched
DDIM-respaced sampling. Denormalize the generated patch-SVD factors (rank=16),
build the reconstructed physical trajectory ``(NX=1024, T_TRAJ=200)`` per
sample, and dump one ``.pt`` per seed.
"""
...
_EXP = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))   # 1d_reaction_diffusion
...
DEFAULT_TRAIN_PATH = ("/work/hdd/bgxp/factor_diffusion/tucker_factors/"
                      "reaction_1d/reaction_1d_train.pt")
...
def pack_test_batch(test_dataset, idxs):
    ...
    return x_flat, cond_flat, nu, rho, sample_idx
...
def generate_one_seed(seed, wrapper, diffusion_sample, test_dataset,
    ...
    idx_chunks, nu_chunks, rho_chunks = [], [], []
    ...
    for b0 in range(0, N, batch_size):
        ...
        _x_flat, cond_flat, nu, rho, sample_idx = pack_test_batch(test_dataset, idxs)
        ...
        samples = diffusion_sample.p_sample_loop(
            ...
            clip_denoised=False,
            model_kwargs={"cond_flat": cond_flat, "nu": nu, "rho": rho},
        ...
        rho_phys = test_dataset.denorm(rho, "log_rho").exp().cpu()
    ...
    return {
        "alpha":      torch.cat(alpha_chunks, dim=0),     # (N, 320, 16)
        "V_hat":      torch.cat(V_hat_chunks, dim=0),     # (N, 640, 16)
        "trajectory": torch.cat(traj_chunks, dim=0),      # (N, 1024, 200)
        "sample_idx": torch.cat(idx_chunks, dim=0),       # (N,)
        "nu":         torch.cat(nu_chunks, dim=0),        # (N,)
        "rho":        torch.cat(rho_chunks, dim=0),       # (N,)
    }
