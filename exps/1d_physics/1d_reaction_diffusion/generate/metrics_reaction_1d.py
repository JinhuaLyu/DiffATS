"""metrics_reaction_1d.py — Per-sample relative-error metrics of generated 1D
Reaction-Diffusion trajectories against the **original physical GT**.

Same structure as 1d_burgers/generate/metrics_burgers_1d.py: load each seed
file's stored trajectory ``(N, 1024, 200)``, compare against
``original tensor[idx, 1:, :].T`` from
``/work/hdd/.../original_data/reaction_1d/reaction_1d_test.pt``, compute
per-sample L1 rel err / L2 rel err / RMSE; aggregate to per-seed mean and
...
def main():
    ...
    orig_tensor = load_orig_test(args.orig_test)
    ...
    print(f"\n===== reaction_1d  epoch={args.epoch}  N_samples={summary['N_samples']}  "
