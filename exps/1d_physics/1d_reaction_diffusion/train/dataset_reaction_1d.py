"""dataset_reaction_1d.py — PyTorch Dataset for 1D Reaction-Diffusion patch-SVD factors.

Loads a SINGLE .pt produced by `data_tucker/save_factors_reaction_1d.py`:

    {
        ...
        "nu"       : (N,)             # diffusion coefficient per sample
        "rho"      : (N,)             # reaction rate         per sample
        ...
    }
...
Normalization (scale-only, mirrors images/our_method/compute_alpha_stats_refimg.py):
    ...
    nu, rho  : log z-score (mean + std). Both used as scalar AdaLN conditioning;
...
class ReactionFactor1DDataset(Dataset):
    """
    Parameters
    ----------
    ...
    def __init__(
        ...
        rho_tensor = rho.float() if isinstance(rho, torch.Tensor) else torch.tensor(rho, dtype=torch.float32)
        self.log_nu_all  = torch.log(nu_tensor)              # (N,)
        self.log_rho_all = torch.log(rho_tensor)             # (N,)
        ...
        assert self.log_rho_all.shape  == (N,)
        ...
        print(
            f"[ReactionFactor1DDataset] split={split}  N={len(self.indices)}"
    ...
    def _compute_stats(self) -> dict:
        ...
        # log(nu) and log(rho) full z-score (scalar); used in AdaLN, not bilinear
        s["log_nu_mean"]   = self.log_nu_all.mean()
        s["log_nu_std"]    = self.log_nu_all.std().clamp(min=1e-8)
        s["log_rho_mean"]  = self.log_rho_all.mean()
        s["log_rho_std"]   = self.log_rho_all.std().clamp(min=1e-8)
        return s
    ...
    def denorm(self, x: torch.Tensor, name: str) -> torch.Tensor:
        """Inverse of __getitem__'s normalization for a given factor tensor or scalar."""
        if name in ("log_nu", "log_rho"):
    ...
    def __getitem__(self, item: int) -> dict:
        ...
        return {
            ...
            "rho":      (self.log_rho_all[idx] - self.stats["log_rho_mean"])
                        / self.stats["log_rho_std"],
            "idx":      idx,
        }
...
if __name__ == "__main__":
    ...
    p = sys.argv[1] if len(sys.argv) > 1 else \
        "/work/hdd/bgxp/factor_diffusion/tucker_factors/reaction_1d/reaction_1d_train.pt"
    ds = ReactionFactor1DDataset(p, split="all")
    s = ds[0]
    for k, v in s.items():
