"""model_burgers_1d_dit.py — Conditional DiT for 1D Burgers patch-SVD factors.
...
AdaLN conditioning: c = t_embedder(t) + nu_embedder(nu_norm).
...
# ---------------------------------------------------------------------------
# Tensor-shape constants  (rank=32 patch SVD on 1024x200 trajectories)
# ---------------------------------------------------------------------------
RANK         = 32
PATCH_DIM    = 640
...
class BurgersDiT1D(nn.Module):
    ...
    def forward(
        ...
        # ── AdaLN signal: timestep + nu ──────────────────────────────────
        c = self.t_embedder(t) + self.nu_embedder(nu)              # (B, H)
...
class Burgers1DWrapper(nn.Module):
    """
    x_flat     : [alpha.flat (10240) | V_hat.flat (20480)]   dim = 30720
    cond_flat  : [alpha_ic.flat (1024) | V_hat_ic.flat (20480)]  dim = 21504
    """
    ...
    def __init__(self, core: BurgersDiT1D):
        super().__init__()
        self.core = core
    ...
    def forward(
        ...
        pred_alpha, pred_V_hat = self.core(alpha, V_hat, alpha_ic, V_hat_ic, t, nu)
...
def build_burgers_1d_dit(cfg: dict) -> Burgers1DWrapper:
    core = BurgersDiT1D(
...
if __name__ == "__main__":
    ...
    nu = torch.randn(B)
    out = wrapper(x, t, cond_flat=cd, nu=nu)
