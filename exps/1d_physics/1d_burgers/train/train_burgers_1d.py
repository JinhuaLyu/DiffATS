"""train_burgers_1d.py — Conditional DDPM training on 1D Burgers patch-SVD factors.
...
Conditions (1D-specific):
  ...
  - scalar log-nu        (AdaLN signal alongside diffusion timestep)
...
def pack(batch):
    """Returns (x_flat, cond_flat, nu).
        x_flat    : (B, FLAT_MAIN=30720) — [alpha | V_hat] normalised
        cond_flat : (B, FLAT_COND=21504) — [alpha_ic | V_hat_ic] normalised
...
def eval_relerr(
    ...
    for s in range(0, n, batch_size):
        ...
        x_flat, cond_flat, nu_batch = pack(batch)
        B = x_flat.shape[0]
...
def main():
    parser = argparse.ArgumentParser(description="Train 1D Burgers Factor DiT")
