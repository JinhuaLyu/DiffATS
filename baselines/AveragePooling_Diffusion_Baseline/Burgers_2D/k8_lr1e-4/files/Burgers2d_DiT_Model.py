...
class GaussianDiffusion(nn.Module):
    ...
    def q_sample(self, x0, t, noise=None):
        """Add noise to x0.  x0: [B, 200, 16, 16]"""
        if noise is None:
    ...
    def training_loss(self, x0, condition):
        """
        x0        : [B, 200, 16, 16]  clean trajectory
        condition : [B,   1, 16, 16]  t=0 frame
        """
        B  = x0.shape[0]
    ...
    @torch.no_grad()
    def generate_trajectory(self, condition_hr, pool_k=8,
                             num_steps=250, orig_size=128):
