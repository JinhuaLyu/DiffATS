...
import wandb
from Burgers1d_avgpooling_dataset import Burgers1dDataset
...
def setup_logger(log_dir, job_id=None):
    ...
    logger = logging.getLogger('burgers1d_train')
...
def make_sample_plot(diffusion, dataset, device,
    ...
    for row, idx in enumerate(indices):
        ...
        cond_nu      = sample['cond_nu'].unsqueeze(0).to(device)
        target       = sample['target'].unsqueeze(0).to(device)

        with torch.amp.autocast('cuda'):
            pred_lr = diffusion.ddim_sample(
                cond_spatial, cond_nu, num_steps=num_ddim_steps
            )  # [1, 200, L_lr]
        ...
        ax.set_title(f'Pred  nu={nu_val:.4f}', fontsize=10)
        ax.legend(fontsize=7)
        ax.set_xlabel('x')
...
def train_one_epoch(diffusion, loader, optimizer, scaler, device,
    ...
    for step, batch in enumerate(pbar):
        ...
        cond_nu      = batch['cond_nu'].to(device,      non_blocking=True)
...
def get_args():
    p = argparse.ArgumentParser(
        description='1D Burgers Conditional Diffusion Training'
    )
    ...
    p.add_argument('--wandb_project',  type=str, default='burgers1d_k4')
...
def main():
    ...
    # model
    L_lr = 1024 // args.pool_k   # 256 for pool_k=4
    ...
    run_name     = args.wandb_name or \
        f"burgers1d_k{args.pool_k}_dim{args.hidden_dim}_fixlr{args.lr}"
