...
import wandb
from Karman2d_avgpooling_dataset import KarmanDataset
...
def setup_logger(log_dir, job_id=None):
    ...
    logger = logging.getLogger('karman_train')
...
def get_args():
    p = argparse.ArgumentParser(
        description='Karman Vortex Trajectory Diffusion Training (Full Preload, Fixed LR)'
    )
    ...
    p.add_argument('--wandb_project',  type=str, default='karman2d_traj_k8')
...
def main():
    ...
    run_name     = args.wandb_name or \
        f"karman_k{args.pool_k}_dim{args.hidden_dim}_fixlr{args.lr}"
