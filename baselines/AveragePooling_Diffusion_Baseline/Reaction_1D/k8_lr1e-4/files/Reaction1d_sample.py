...
def get_args():
    ...
    p.add_argument('--test_path', type=str,
        default='/anvil/projects/x-eng260004/factor_diffusion/pde_samples/1d_diffusion/pde_samples_10.pt')
    p.add_argument('--ckpt_path', type=str,
        default='/anvil/projects/x-eng260004/factor_diffusion/baseline_checkpoint/average_pooling/Reaction1d_k8_latest.pt')
    p.add_argument('--output_path', type=str,
        default='/anvil/projects/x-eng260004/factor_diffusion/pde_samples_generated/1d_diffusion/AveragePooling/avgpool_reaction1d_generated.pt')
...
def main():
    ...
    cond_params_all = torch.stack([nu_all, rho_all], dim=1)  # [N, 2]
    ...
    for i in tqdm(range(N), desc='sampling'):
        ...
        cond_pr = cond_params_all[i:i+1].to(device)
        with torch.no_grad(), torch.amp.autocast('cuda'):
            pred_lr = diffusion.ddim_sample(cond_sp, cond_pr, num_steps=args.ddim_steps)
