...
@torch.no_grad()
def evaluate_sample(diffusion, cond_spatial, cond_params, gt_hr,
                    num_steps, orig_L, device):
    with torch.amp.autocast('cuda'):
        pred_lr = diffusion.ddim_sample(
            cond_spatial, cond_params, num_steps=num_steps
        )  # [1, 200, L_lr]
...
def main():
    ...
    L_lr = args.orig_L // args.pool_k   # 128 for pool_k=8
    ...
    rho_all      = data['rho'][:args.num_test_samples]      # [500]

    cond_spatial_all = F.avg_pool1d(
        trajectories[:, 0:1, :], kernel_size=args.pool_k
    )  # [500, 1, L_lr]

    # GT: t=1..200 at original resolution
    gt_hr_all = trajectories[:, 1:, :]   # [500, 200, 1024]

    # concat [nu, rho] -> [500, 2]
    cond_params_all = torch.stack([nu_all, rho_all], dim=1)
    ...
    for seed_idx in range(args.num_seeds):
        ...
        for i in tqdm(range(len(trajectories)), desc=f'Seed {seed_idx+1}'):
            ...
            cond_pr = cond_params_all[i:i+1].to(device)     # [1, 2]
            gt_hr   = gt_hr_all[i].to(device)               # [200, 1024]

            m = evaluate_sample(
                diffusion, cond_sp, cond_pr, gt_hr,
