...
# Physics Params Embedder: [nu, rho] -> hidden_dim

class ParamsEmbedder(nn.Module):
    def __init__(self, hidden_dim, in_dim=2):
        super().__init__()
        self.mlp = nn.Sequential(
...
# 1D TrajDiT for Reaction-Diffusion
...
class TrajDiT1D(nn.Module):
    ...
    def __init__(
        ...
        # condition spatial embedder: 1 frame -> hidden_dim
        self.cond_embedder = nn.Linear(1, hidden_dim)

        # 1D positional embedding for L_lr + 1 tokens
        ...
        # physics params embedder: [nu, rho] -> hidden_dim
        self.params_embedder = ParamsEmbedder(hidden_dim, in_dim=2)
    ...
    def forward(self, noisy_target, cond_spatial, cond_params, t):
        # tokenise target: [B, 200, L_lr] -> [B, L_lr, 200] -> [B, L_lr, D]
        target_tokens = noisy_target.permute(0, 2, 1)
        target_tokens = self.target_embedder(target_tokens)       # [B, L_lr, D]

        # tokenise cond spatial: mean over spatial -> 1 token [B, 1, D]
        ...
        # concat: [cond_token | target_tokens] -> [B, L_lr+1, D]
        tokens = torch.cat([cond_token, target_tokens], dim=1)
        tokens = tokens + self.pos_embed                          # [B, L_lr+1, D]

        # conditioning: diffusion t + [nu, rho]
        c = self.t_embedder(t) + self.params_embedder(cond_params)  # [B, D]

        # transformer blocks
        for block in self.blocks:
            tokens = block(tokens, c)

        # final layer on target tokens only
        target_out = self.final_layer(tokens[:, 1:, :], c)        # [B, L_lr, 200]

        return target_out.permute(0, 2, 1)                        # [B, 200, L_lr]
...
class GaussianDiffusion(nn.Module):
    ...
    def training_loss(self, target, cond_spatial, cond_params):
        B = target.shape[0]
        t = torch.randint(0, self.timesteps, (B,), device=target.device)
        noisy_x, noise = self.q_sample(target, t)
        pred_noise = self.model(noisy_x, cond_spatial, cond_params, t)
        return F.mse_loss(pred_noise, noise)

    @torch.no_grad()
    def ddim_sample(self, cond_spatial, cond_params, num_steps=250, eta=0.0):
        ...
        for i, t_val in enumerate(timesteps):
            ...
            pred_noise = self.model(x, cond_spatial, cond_params, t)
...
if __name__ == '__main__':
    ...
    B = 4
    target       = torch.randn(B, 200, 128)
    cond_spatial = torch.randn(B, 1, 128)
    cond_params  = torch.rand(B, 2)   # [nu, rho]

    loss = diffusion.training_loss(target, cond_spatial, cond_params)
    print(f"Train loss : {loss.item():.4f}")

    gen = diffusion.ddim_sample(cond_spatial, cond_params, num_steps=10)
    print(f"Generated  : {gen.shape}")   # [B, 200, 128]
    print("Model ready.")
