...
def LSimple_cond(score_model: ScoreModel, x0, n_cond, pred='noise_pred', reweight=None, **kwargs):
    """Conditional training loss (RePaint-style): first n_cond tokens are kept
    clean in the noisy input; loss is computed only on the remaining tokens."""
    t, noise, xt = score_model.sde.sample(x0)
    xt = xt.clone()
    xt[:, :n_cond] = x0[:, :n_cond]
    if pred == 'noise_pred':
        noise_pred = score_model.noise_pred(xt, t, **kwargs)
        loss = (noise - noise_pred).pow(2)
        if reweight is not None:
            loss = loss * reweight
        loss = loss[:, n_cond:]
        return loss.flatten(start_dim=1).mean(dim=-1)
