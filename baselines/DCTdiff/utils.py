...
from DCT_utils import idct_transform, combine_blocks, combine_blocks_3d, idct3_block
import threading
import queue
...
def DCT3D_samples_to_clips(samples, tokens, low_freqs, block_T, block_H, block_W,
                           reverse_order_3d, T, H, W, Y_bound):
    """Decode (B, tokens, low_freqs) DCT samples back to (B, T, H, W) clips."""
    samples = np.asarray(samples, dtype=np.float32)
    samples = np.clip(samples, -2, 2)
    bound = float(np.asarray(Y_bound, dtype=np.float32).reshape(-1)[0])

    B = samples.shape[0]
    coefs_per_block = block_T * block_H * block_W
    full = np.zeros((B, tokens, coefs_per_block), dtype=np.float32)
    full[:, :, :low_freqs] = samples
    full = full[:, :, reverse_order_3d]
    full = full * bound

    clips = np.empty((B, T, H, W), dtype=np.float32)
    for b in range(B):
        blocks = full[b].reshape(tokens, block_T, block_H, block_W)
        decoded = np.empty_like(blocks)
        for j in range(tokens):
            decoded[j] = idct3_block(blocks[j])
        clips[b] = combine_blocks_3d(decoded, T, H, W, block_T, block_H, block_W)
    return clips


def _scalar_field_to_rgb(field):
    field = np.asarray(field, dtype=np.float32)
    max_abs = max(float(np.max(np.abs(field))), 1e-8)
    norm = np.clip((field / max_abs + 1.0) * 0.5, 0.0, 1.0)
    red = norm
    blue = 1.0 - norm
    green = 1.0 - np.abs(2.0 * norm - 1.0)
    rgb = np.stack([red, green, blue], axis=-1)
    return np.uint8(np.clip(rgb * 255.0, 0, 255))


def DCT3D_samples_to_video_grid(samples, tokens, low_freqs, block_T, block_H=None, block_W=None,
                                block_HW=None, reverse_order_3d=None, T=None, H=None, W=None,
                                Y_bound=None, n_rows=8, frames_per_row=8, path=None):
    """Render (B, tokens, low_freqs) samples as a grid: each row is one clip's
    evenly-spaced frames rendered as vorticity heatmaps.

    Accepts either (block_H, block_W) or the square shorthand block_HW."""
    if block_HW is not None:
        block_H = block_W = block_HW
    samples = samples.detach().cpu().numpy()
    samples = samples[:n_rows]
    clips = DCT3D_samples_to_clips(
        samples, tokens, low_freqs, block_T, block_H, block_W,
        reverse_order_3d, T, H, W, Y_bound,
    )

    frame_idx = np.linspace(0, T - 1, frames_per_row).round().astype(int)
    pad_h, pad_v = 2, 2
    rows = []
    for b in range(clips.shape[0]):
        frames = clips[b, frame_idx]
        rendered = [_scalar_field_to_rgb(f) for f in frames]
        gap_v = np.full((H, pad_v, 3), 255, dtype=np.uint8)
        parts = []
        for k, f in enumerate(rendered):
            parts.append(f)
            if k < frames_per_row - 1:
                parts.append(gap_v)
        rows.append(np.concatenate(parts, axis=1))

    row_w = rows[0].shape[1]
    gap_h = np.full((pad_h, row_w, 3), 255, dtype=np.uint8)
    out_parts = []
    for k, r in enumerate(rows):
        out_parts.append(r)
        if k < len(rows) - 1:
            out_parts.append(gap_h)
