...
def main():
    ...
    T0, H0, W0 = first[0][FIELDS[0]].shape
    print(f"  shard[0][0].{FIELDS[0]}: ({T0}, {H0}, {W0})  (will crop T -> {T_USE})")
    ...
    n_channel_clips = clips_per_shard * len(shard_paths) * len(FIELDS)
    print(f'  clips/shard = {clips_per_shard}, fields = {FIELDS}, '
          f'total channel-clips for stats = {n_channel_clips}')
    ...
    for s_idx, sp in enumerate(shard_paths):
        ...
        # release shard memory before the next ~2.6 GB load
        del shard_obj
        if s_idx == 0:
            first = None
    ...
    n_use = count // num_blocks_per_clip
    stats = {
        'Y_bound': y_bound,
        'vor_std': vor_std,
        'T_use': T_USE,
