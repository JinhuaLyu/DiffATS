...
def plot_generate_git(basis, core, core_mean, core_std, data_std, data_mean, u_ind_uni, v_ind_uni, file_name = r"generate_animation.gif"):
    ...
    n_frames = output_batch.shape[0]  # Number of frames
    vmin = np.min(data[:n_frames,:,:])
    vmax = np.max(data[:n_frames,:,:])
    # Initialize image (start by drawing the first frame)
    im = ax.imshow(data[0], cmap='viridis', vmin=vmin, vmax=vmax)
    # Update function called every frame
    def update(frame):
        im.set_array(data[frame])  # Update displayed frame
        return [im]

    # Build animation; `frames` = number of frames, `interval` = ms between frames
