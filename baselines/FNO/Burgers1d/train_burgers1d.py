...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--time_budget_seconds", type=float, default=17000.0)
    p.add_argument("--rollout_eval_batch", type=int, default=64,
                   help="batch size for full-rollout evaluation on test set")
    p.add_argument("--ckp_dir", default="/gpfs/home/bkx8728/Tensor_factor/1dscripts/fno_burgers/ckp")
    p.add_argument("--run_name", default="fno_burgers_1d_ar")
...
def load_full(path: str):
    ...
    print(f"[data] traj {tuple(traj.shape)}  log_nu {tuple(log_nu.shape)}", flush=True)
    return traj, log_nu
...
def train_one_epoch(model, traj_n, log_nu_n, optimizer, h1_loss, batch_size, device, epoch, time_start, time_budget, use_amp=False, subsample_t=0):
    N, T, X = traj_n.shape
    if subsample_t == -1:
        total_samples = N
    ...
    for step in range(iters):
        ...
        nu_ch = log_nu_n[n_idx].unsqueeze(-1).expand(-1, X)

        x = torch.stack([u_in, nu_ch], dim=1)
        y = u_out.unsqueeze(1)
...
@torch.no_grad()
def rollout_eval(model, traj_n, log_nu_n, h1_loss, l2_loss, batch_size, device, use_amp=False):
    model.eval()
    N, T, X = traj_n.shape
    ...
    for batch_start in range(0, N, batch_size):
        ...
        nu_ch = log_nu_n[batch_start:batch_end].unsqueeze(-1).expand(-1, X)
        gt = traj_n[batch_start:batch_end]

        traj_pred = [u_curr]
        for t in range(T - 1):
            x = torch.stack([u_curr, nu_ch], dim=1)
            if use_amp:
                with torch.amp.autocast(device_type="cuda", dtype=torch.bfloat16):
    ...
    avg_h1_per_step = (h1_per_step / N * batch_size).cpu()
...
def main():
    ...
    traj_tr, lnu_tr = load_full(args.train_path)
    traj_te, lnu_te = load_full(args.test_path)
    ...
    if args.compile:
        try:
            ...
            print("[setup] torch.compile mode='default' enabled", flush=True)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        train_loss, stopped = train_one_epoch(
            model, traj_tr_n, lnu_tr_n, optimizer, h1_loss,
        ...
        if run_eval:
            ...
            h1_step, l2_step, full_l2 = rollout_eval(
                model, traj_te_n, lnu_te_n, h1_loss, l2_loss,
